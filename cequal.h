...
inline uint32_t uint32_bit_reverse(uint32_t x, uint32_t width)
{
...
inline void Ceq_Man_t::run_fft(std::vector<Cpx_t>& vCpxIn, std::vector<Cpx_t>& vResult, const std::vector<Cpx_t>& vDataIn, std::vector<Cpx_t> * pvCpx, bool fifft){
	...
	uint32_t pow2 = 0;	
	for(int i = size_of_window; i > 0 ; i >>=1, pow2 ++ )
		assert( !(i&1) || 1==i );
	pow2 -= 1;
	...
	for(int i = 0; i < size_of_window; i ++){
		int idx = uint32_bit_reverse(i,pow2);
		vCpxIn[ i ]    = vDataIn[ idx ];
	}
